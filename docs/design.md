...
**Why.** Locked in CLAUDE.md commitment #3. Sonnet 4.6 is the workhorse (strong reasoning, moderate cost). The Gateway abstraction makes the provider swappable for EPFL's on-prem path (RCP / local LLMs).
...
A discriminated union over operation kinds. The discriminator is the `operation` literal field. The common envelope is shared via a base class. **In v1.0, only `REPLACE_FIELD` is produced by the planner and applied by the propose stage.** The other four operation types (`UPDATE_POINTS`, `ADD_NODE`, `REMOVE_NODE`, `REORDER_NODES`) are defined in the schema for extensibility but are not currently emitted.
...
**Invariant**: every `ProposedChange` with `application_status = APPLIED` is reflected in `improved_rubric`. The three-step application pipeline (§ 5.5 DR-IM-07) enforces this. In v1.0, only `REPLACE_FIELD` operations are produced, so step 1 (conflict resolution) is a no-op and step 3 applies field replacements only.
...
**Grader simulation** (`grading_rubric.assess.simulation`). The simulation assembles a response set (real student texts + synthetic responses to reach `Settings.assess_target_response_count`, default 10), then grades every response with 4 fixed personas:
...
- **AmbiguityEngine**: for each `(response, criterion)`, computes persona grade stdev. Midscale disagreement (stdev ≥ 0.10 and extremity < 0.40) is an ambiguity signal. Finding emitted when `problem_rate ≥ 0.20`. Confidence floored to 0.20 when fewer than 3 midscale opportunities. Method: `LLM_PANEL_AGREEMENT`.
...
- **DiscriminationEngine**: two sub-methods. (a) **Score distribution separation**: calibration score (`1.0 - 2×mean_error`), rank correlation (Spearman, rescaled to [0,1]), ceiling-effect penalty, combined with weights `(0.25 cal, 0.20 rank, 0.15 pairwise, 0.40 ceiling)`. (b) **Pairwise consistency**: near-equal absolute scores but clear pairwise winner → discrimination finding (with optional linked ambiguity finding per SR-AS-10 dual signal).

**Headline scores** (`scores_from_simulation`): per-criterion scores averaged across criteria, all using `QualityMethod.GRADER_SIMULATION`. Both assess and score stages compute scores from the simulation; assess stores them as `AssessOutputs.quality_scores` (used as `previous_quality_scores` in the deliverable), score computes them against the improved rubric.
...
```
  ...
  │     → (N × 4 × C) gateway.measure() calls, temp 0.7    │
...
| **DR-AS-05** | Must | Confidence per `AssessmentFinding` is computed inline by each engine. Ambiguity findings: `max(0.20, min(0.90, 1.0 - problem_rate))`, lowered to `0.20` when fewer than 3 midscale responses exercised ambiguity. Applicability findings: `max(0.20, min(0.85, 1.0 - problem_rate))`. Discrimination spread findings: constant `0.65`. Discrimination pairwise findings: constant `0.60`. Ambiguity-attributed pairwise findings: constant `0.55`. The **floor of `0.20`** preserves LOW-confidence honesty without forcing decorative zeros. The `ConfidenceIndicator.rationale` string is **mandatory and human-readable**, naming the signal source (e.g. *"inferred from midscale grade disagreement across personas"*). Per-criterion `CriterionScore.confidence` on the deliverable is a separate concern owned by `scores_from_simulation()` and the score stage (DR-SCR-02). | SR-AS-08, § 4.5, DR-SCR-02 |
...
**v1.0 scope.** Only the **modify-existing** path is implemented: the LLM planner proposes `REPLACE_FIELD` edits grounded in assessment findings. The generate-from-scratch path (DR-AS-15 sentinel) and the LLM-driven grounding-check / narrative-assembly calls described in the DR table below are **defined for extensibility but not implemented in v1.0** — the planner is the single LLM call. Other operation types (`ADD_NODE`, `REMOVE_NODE`, `UPDATE_POINTS`, `REORDER_NODES`) are schema-defined but the planner does not produce them.
...
```
   ...
   │  3. Apply REPLACE_FIELD + wrap into ProposedChange  │
   │     source_operations = []                          │
   └─────────────────────────────────────────────────────┘
                              │
                              ▼
...
This group is the **thin DR layer** that locks the L4 SPA against the rest of the design. The visual specification of the three screens — *Input*, *Running*, *Review* — lives in [`ui-draft.md`](ui-draft.md) v1.0.0 § 4 and is normative there; the present group does **not** repeat it. What this group locks are the contracts the SPA must satisfy: the technology stack and isolation invariants, the screen scope and one-way navigation, the state-management discipline (no SPA-side persistence, hermetic per session), the wiring from the *Input* screen to the registered Validance workflow, the *Running* screen polling cadence and label-mapping table, the side-by-side diff component contract against the `ProposedChange` discriminated union of § 4.6, the per-change accept/reject controls and re-run loop wired through Validance's `ApprovalGate`, and the teacher-native language discipline that governs every label, status, and *Why?* affordance content shape. Together with `ui-draft.md` § 4 the present group is the deliverable's full UI specification; the visual document covers the *what*, the present group covers the *what it must satisfy*.
...
The `score` stage re-runs `run_grader_simulation()` against the improved rubric (reusing the response set from assess when available) and calls `scores_from_simulation()` to produce headline `CriterionScore` records — both functions live in `grading_rubric.assess` and are shared with the assess stage. The `Scorer` Protocol and `scorer_backend` setting exist for future extensibility (a trained model scorer), but **only `grader_simulation` is used in v1.0** — the `LlmPanelScorer` raises `RuntimeError` (deprecated) and `TrainedModelScorer` is a stub. This group also defines the stub `train_scorer` task per locked architectural commitment #5: the hiring team evaluates whether the candidate can build a system that has a train button and a replaceable scoring backend.
...
| ID | Criticality | Statement | Trace |
|---|---|---|---|
| **DR-SCR-01** | Must | The `scorer` sub-package declares a `Scorer` Protocol with one method: `score_rubric(rubric, evidence, settings, audit_emitter) -> ScoringResult`. In **v1.0**, the `score_stage` entry point bypasses this Protocol and directly calls `run_grader_simulation()` + `scores_from_simulation()` (shared with assess), producing `ScoreOutputs` with `quality_scores: list[CriterionScore]` on the improved rubric plus `previous_quality_scores` carried from `AssessOutputs.quality_scores` for before/after comparison. The `Scorer` Protocol exists for future extensibility (a trained model would implement it). | § 2.4, § 4.5, SR-AS-09 |
| **DR-SCR-02** | Must | The default score stage re-runs the shared grader simulation against the improved rubric, reusing the same response set when available, and converts the resulting grade matrix into one `CriterionScore` per quality criterion. Each score uses `QualityMethod.GRADER_SIMULATION` and links back to the simulation operation evidence through `source_operation_id` when available. The historical `LlmPanelScorer` compatibility shell is not on the main scoring path. | § 4.5, § 4.9, DR-LLM-01, DR-LLM-08, SR-AS-01, SR-AS-02, SR-AS-03 |
...
| **SR-OUT-01** — explained rubric file as deliverable | DR-DAT-07 |
| **SR-OUT-02** — root JSON with improved rubric + explanation | DR-DAT-07 |
...
| **SR-OBS-01** — record audit bundle per run | DR-ARC-06, DR-OBS-01, DR-INT-03, DR-INT-05 |
...
Every DR shall trace back to at least one SR. Every SR shall be covered by at least one DR. The matrix above closes both directions of the chain at v0.19.0; future DR additions update both the per-DR `Trace` column and this matrix in lockstep.
...
Detailed round-by-round history archived in `CLAUDE.md` § 10.
