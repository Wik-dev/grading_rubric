...
- Deployment, packaging, orchestration — including how the four-layer split (L1 Python package + CLI / L2 Docker / L3 Validance integration / L4 Lovable SPA) lets the same hermetic L1 stages run as direct CLI invocations on Path A or as Validance workflow tasks on Path B.
...
```
                  ...
                  v                                                       v
   +---------------------------+                       +----------------------------------+
   |  Path A: CLI direct       |                       |  Path B: Validance + Lovable SPA |
...
| 10 | Deployment topology, packaging | **decided** (revised v0.14.0) | Four-layer deliverable: **L1** Python package `grading_rubric` installable via `pyproject.toml` with one CLI subcommand per pipeline stage (`grading-rubric-cli <stage> ...`); **L2** Docker images baking L1 + the CLI as their entrypoint; **L3** `validance/` integration directory containing workflow definitions, registration script, proposal-payload mapping, and audit-chain harvester; **L4** `frontend/` Lovable-built SPA build talking to Validance's REST API. **No custom HTTP server in the deliverable.** Top-level `Makefile` is the entry-point surface (`install` / `images` / `register` / `dev` / `build` / `test` / `schemas`). | § 3.10 |
...
4. **L4 — `frontend/` Lovable SPA build.** Custom Vite + React + shadcn + Tailwind SPA built with Lovable, talking to **Validance's REST API** as its backend. Built with `npm install && npm run build`; the build artefact is a static `frontend/dist/` that any static host can serve (and that Validance itself can serve for the demo path).
...
- **Why Lovable for L4.** Locked in CLAUDE.md commitment #1. The Lovable scaffold (Vite + React + shadcn + Tailwind) is the standard way the user builds custom SPAs in the parent repo group; the SPA reverse-engineering loop from `notes/dr-r3-draft.md` § A (write functional spec → let Lovable generate → write DR-UI from observed code) is the methodology for filling DR-UI later.
- **Why no `make run` convenience target.** The full demo path is "run `make register` once to register the workflow against the hosted Validance instance, then open the Lovable SPA in a browser". There is no single process to start in the deliverable repository; the things that need to be running (Validance API, PostgreSQL, the L2 worker images) are running on the hosted dev VM, not on the reviewer's host. The single-stage inspection path is `docker run <image> grading-rubric-cli <stage> ...` and is documented in the README as a one-liner.
