...
**Version**: 0.14.0
**Date**: 2026-04-11
**Status**: **Validance pivot folded in.** Architectural overview rewritten around the four-layer architecture (L1 Python package + CLI / L2 Docker / L3 Validance integration / L4 Lovable SPA); all 11 technology-stack decisions locked, with §§ 3.10–3.11 rewritten to reflect Validance as the recommended runner and the hosted dev-VM instance as the canonical demo target; § 4 data models preserved with § 4.8 reframed as a *typed view* of Validance's audit chain populated by the L3 harvester; **DR-ARC, DR-DAT, DR-DEP, and DR-INT filled (≈ 35 DRs across the four groups; DR-ARC-07 repurposed to lock the L1 zero-Validance-imports invariant; DR-DAT-07 / DR-DAT-10 surgically rewritten to drop the `runs/<run_id>/` layout; § 5.12 *DR-INT* added)**; remaining DR groups (DR-LLM, DR-AS, DR-IM, DR-UI, DR-IO, DR-OBS, DR-PER, DR-MOD) still to be filled.
...
| **DR-INT-03** | Must | Every L1 stage that calls the LLM (per DR-LLM-01) emits its operation events via the `audit_emitter` to `stderr` as structured-JSON lines (per DR-ARC-06 case (a)) when invoked through Validance. The L3 task wrapper captures this stderr stream as part of Validance's standard task-output capture; the harvester (DR-INT-05) joins it with Validance's per-task audit-chain rows to build the typed `AuditBundle` view. No private file format crosses the L1↔L3 boundary; the contract is *structured stderr lines + Validance's own audit chain*. | § 4.8, DR-ARC-06 |
