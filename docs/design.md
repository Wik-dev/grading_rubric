...
**Version**: 0.15.0
**Date**: 2026-04-11
**Status**: **R3 cluster (DR-LLM + DR-OBS) folded in post-pivot.** Architectural overview rewritten around the four-layer architecture (L1 Python package + CLI / L2 Docker / L3 Validance integration / L4 Lovable SPA); all 11 technology-stack decisions locked, with §§ 3.10–3.11 reflecting Validance as the recommended runner and the hosted dev-VM instance as the canonical demo target; § 4 data models preserved with § 4.8 reframed as a *typed view* of Validance's audit chain populated by the L3 harvester. **6 of 12 DR groups filled** (≈ 50 DRs total): § 5.1 *DR-ARC* (12), § 5.2 *DR-LLM* (11), § 5.3 *DR-DAT* (11, with DR-DAT-03 extended to regenerate the `audit_event.v1.schema.json` wire schema), § 5.8 *DR-OBS* (4, locking the closed five-event-kind set, the wire format, the privacy boundary, and a test-helper), § 5.11 *DR-DEP* (9), § 5.12 *DR-INT* (9, with DR-INT-03 now referencing DR-OBS-01 by ID for the wire-schema contract). Remaining DR groups (DR-AS, DR-IM, DR-UI, DR-IO, DR-PER, DR-SCR) still to be filled.
...
| **DR-ARC-08** | Must | The `cli` sub-package exposes a single console-script entry point `grading-rubric-cli` with **one subcommand per pipeline stage** plus a thin orchestrator subcommand: `ingest`, `parse-inputs`, `assess`, `propose`, `score`, `render`, and `run-pipeline`. Each per-stage subcommand reads its structured input from disk (a JSON / Pydantic-validated file or a directory of inputs declared by the stage's `stage_inputs` schema), invokes the stage callable through the `Stage` protocol, and writes the structured output to a path the user specifies; this is the unit of single-stage inspection on Path A. `run-pipeline` chains the stages in order via the in-process `orchestrator` (DR-ARC-04) and is the same code path the test suite exercises. The CLI is a thin shell — every per-subcommand handler is at most input validation, calling the stage callable, and writing the result. | § 3.10, § 3.11 |
...
| **DR-INT-02** | Must | The Validance workflow registered by `register.py` is `grading_rubric.assess_and_improve`, with **one Validance task per L1 pipeline stage**: `ingest` → `parse_inputs` → `assess` → `propose` → `approve` (an `ApprovalGate`, see DR-INT-06) → `score` → `render`. Each L1 stage is wrapped as a Validance task whose execution is `docker run <image> grading-rubric-cli <stage> --input … --output …` with the `<image>` from L2 (DR-DEP-03) and the input/output paths declared on the task's `inputs` / `outputs` schema. The Validance workflow definition does **not** call any L1 Python function in-process; the only L1↔Validance contact surface is the CLI subcommand exit code, the structured `--output` file, and the structured stderr operation events that the harvester consumes. | § 3.11, DR-ARC-08, DR-DEP-06 |
...
| **DR-INT-06** | Must | Human-in-the-loop is realized by a Validance `ApprovalGate` task placed between `propose` and `score` in the workflow definition of DR-INT-02. The gate's `proposal_payload` is the list of `ProposedChange` items produced by `propose`, mapped via DR-INT-04. The teacher reviews the changes in the L4 SPA (which receives Validance's approval-event webhook), accepts or rejects each change individually, and either (a) closes the gate to let `score` and `render` run on the accepted set (single-pass run, SR-AS-09 not exercised) or (b) requests a re-measurement iteration, which causes Validance to re-enter the `assess` → `propose` → `approve` segment with the updated rubric — capped by `Settings.max_iterations` (default `3`) as a **safety bound**, never as an autonomous convergence loop (§ 2.5). Every iteration of the loop is gated by an explicit teacher decision; the system does not iterate on its own. | § 2.5, UR-07, UR-08, SR-AS-09, SR-UI-09, SR-UI-10, SR-OUT-05 |
