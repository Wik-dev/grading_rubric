...
**Version**: 0.16.1
**Date**: 2026-04-11
**Status**: **R4 cluster (DR-IO + DR-PER + DR-SCR) folded in, plus three Codex review passes absorbed; v0.16.1 patch drops the development-tool name from the body so the SPA is described by its technology stack alone.** Architectural overview rewritten around the four-layer architecture (L1 Python package + CLI / L2 Docker / L3 Validance integration / L4 SPA); all 11 technology-stack decisions locked, with §§ 3.10–3.11 reflecting Validance as the recommended runner and the hosted dev-VM instance as the canonical demo target; § 4 data models preserved with § 4.8 reframed as a *typed view* of Validance's audit chain populated by the L3 harvester, and the small post-review additions of `EvidenceProfile.synthetic_responses_used` (DR-IO / SR-AS-06) and `LlmCallDetails.rate_limit_retries` (DR-LLM-07). **9 of 12 DR groups filled** (79 DRs total): § 5.1 *DR-ARC* (12, with DR-ARC-08 extended to an eighth `train-scorer` CLI subcommand), § 5.2 *DR-LLM* (11), § 5.3 *DR-DAT* (11), § 5.7 *DR-IO* (8), § 5.8 *DR-OBS* (4), § 5.9 *DR-PER* (8), § 5.10 *DR-SCR* (7, with the locked-commitment carve-out from the universal SR-trace rule documented both in the group intro and in § 6), § 5.11 *DR-DEP* (9), § 5.12 *DR-INT* (9, with DR-INT-02 now registering two Validance workflows — `assess_and_improve` and the separate `train_scorer`). Remaining DR groups: § 5.4 *DR-AS*, § 5.5 *DR-IM*, § 5.6 *DR-UI*.
...
**Path A (CLI direct, single-stage inspection)** — `docker run <image> grading-rubric-cli <stage> --input ... --output ...`. The reviewer runs one stage of the pipeline at a time, sees its inputs and outputs on disk, and inspects what each stage does in isolation. There is no audit chain, no approval gate, and no SPA on this path. It is the *single-stage debug surface* and is what makes the hermetic-task claim verifiable in 30 seconds.

**Path B (Validance + SPA, full V-model experience)** — the teacher (or reviewer) opens the SPA in a browser; the SPA calls Validance's REST API; Validance schedules the pre-registered workflow against L2 images; the workflow runs `ingest` → `parse_inputs` → `assess` → `propose` → `approve` (gate) → `score` → `render`; the teacher reviews proposed changes through the SPA via Validance's `ApprovalGate`; the deliverable JSON comes back; the audit chain is populated by Validance and harvested into the deliverable's optional provenance section. This is the canonical demo target and the path against which the V-model is exercised end to end.

**Key cross-layer rules** (locked in CLAUDE.md commitment #4):
...
Each pipeline stage in L1 (`grading_rubric/`) is built as a **hermetic task**: it takes structured inputs, produces structured outputs, holds no global state, performs no hidden I/O outside its declared inputs and outputs, and is reachable from three execution surfaces using the same code:

1. **Direct CLI invocation** — `grading-rubric-cli <stage> --input ... --output ...`. One stage at a time, single-stage inspection. This is the *Path A* surface from § 2.1 and the testbed during development.
2. **In-process stage chain** — the thin `orchestrator` sub-package walks all stages sequentially in one Python process (`grading-rubric-cli run-pipeline ...`). Used by tests and by the developer during local iteration. It is **not** the production orchestrator.
3. **Validance workflow** — the L3 integration directory (§ 5.12 *DR-INT*) defines a Validance workflow whose `command:` attributes invoke the **same CLI commands** as surface (1), running inside the **same L2 Docker images** that surface (1) uses. This is the *Path B* surface from § 2.1 and the canonical demo target.

The three surfaces share one CLI definition. Adding Validance does not double-implement anything in L1. Removing Validance and writing an integration directory for a different orchestrator (Snakemake, Airflow, anything else) is a self-contained change in a new top-level directory; L1 does not move.

What this gives the project: every stage is independently testable, every run is replayable from its declared inputs, and the *cross-cutting concerns that the V-model cares about* — audit chain, approvals, retries, provenance, the SPA backend — are delegated to whichever orchestrator the L3 integration directory targets. In the default case that orchestrator is **Validance**, and those concerns are realized by Validance's own primitives instead of being reinvented inside L1.
...
- **Classical NLP and statistics inserted wherever they are strictly better** — inter-rater reliability via Krippendorff's α, deterministic span matching for the side-by-side rubric diff. The LLM is used where its semantic flexibility is the right tool, not as the default for everything.
...
- **Per-change accept/reject is realized by Validance's `ApprovalGate` primitive** (UR-07 → SR-UI-09, SR-OUT-05). On *Path B* (§ 2.1), the `propose` stage emits `ProposedChange` instances; the L3 integration maps each to a Validance proposal payload; Validance surfaces them through its REST API and the L4 SPA renders them; the teacher accepts or rejects each through the SPA; the resolved decisions are then handed to the `score` and `render` stages downstream of the gate. The deliverable's `ExplainedRubricFile.proposed_changes[*].teacher_decision` field reflects the gate outcome (SR-OUT-05). The L1 CLI (Path A) does not surface approval — it auto-applies all proposed changes (or none, controlled by a CLI flag) since the CLI path is single-stage inspection, not the full V-model experience.
- **Re-measurement after teacher edits is teacher-gated, not autonomous** (UR-08 → SR-AS-09, SR-UI-10). On Path B, after the teacher reviews proposed changes through the gate and the `score` stage produces the new quality scores, the SPA shows the before/after evidence and the teacher decides whether to re-iterate. Only on an explicit "re-run" action from the teacher does the workflow re-enter `assess` with the updated rubric. The system never auto-converges; `Settings.max_iterations` (default `3`) is a safety bound that caps how many iterations the loop *can* perform within one Validance run, not a counter the system runs to. The full iteration trajectory lives in the audit chain that Validance captures and the L3 harvester adapts into `IterationSnapshot` entries on the deliverable's optional provenance section.
...
Locked architectural commitments from `CLAUDE.md` § 6 (not re-litigated here): Anthropic as the default LLM provider; pluggable backend; **Validance as the recommended runner of the four-layer architecture** (commitment #4 revised v0.14.0 — see `notes/validance-pivot.md`); the hosted dev-VM Validance instance as the canonical demo target; tasks remain hermetic and CLI-callable directly.
...
The SPA is a **separate deployable** with **no custom Python HTTP server in the deliverable** (the v0.14.0 pivot — see § 3.10 / § 3.11 / DR-ARC-10). It talks directly to **Validance's REST API** as its backend; the wire types it consumes are JSON shapes generated from L1's Pydantic models via the `make schemas` codegen path of DR-DAT-03 / DR-DAT-04, and the structured outputs of L1 stages reach the SPA through Validance's standard task-output-capture surface and the L3 harvester (DR-INT-05). The front-end reads the Validance base URL from a Vite environment variable (`VITE_VALIDANCE_BASE_URL`) so the same build runs against the hosted dev-VM Validance instance, a local Validance instance, or any other Validance deployment without code changes.

The specific deployment topology of the four-layer split — including the explicit absence of a custom Python HTTP server in L1 — is the subject of decision **#10** (§ 3.10); this decision locks only the front-end stack and the boundary (Validance REST as the SPA's backend).
...
- **Why the SPA → Validance REST boundary.** Keeping the UI as a standalone build that talks to Validance's REST API matches the hermetic-task philosophy (§ 2.2) and the four-layer split locked in § 3.10: each L1 stage is testable in isolation through the CLI (Path A); the SPA reaches the same code paths through Validance task wrappers and the L3 harvester (Path B); and **no custom Python HTTP server** has to be invented to glue the two sides together because Validance already exposes one. It also matches the architecture of the adjacent projects in this repository group, so the deployment story in § 5.11 *DR-DEP* and § 5.12 *DR-INT* is a direct application of patterns already in use.
- **What this decision does not commit.** The Python HTTP server question is *answered* by decision **#10** post-pivot — there is **no custom Python HTTP server** in the L1 deliverable; the SPA (L4) talks to Validance's REST API as its backend (§ 3.10, § 3.11, DR-ARC-10). Authentication is not in scope at the L1 level per [`requirements.md`](requirements.md) § 1.2 *out of scope*; on Path B, Validance's own access controls mediate any sharing of run state. Multi-page vs. multi-route navigation inside the SPA is a DR-UI concern, not a technology-stack concern, and is deferred to § 5.6.
...
- **What this decision does not commit.** OCR for handwritten student copies is decision **#5** (§ 3.5). Whether the application caches parsed documents at all is decision **#8** (§ 3.8) — and the answer there is *no application-level cache*. The shape of the `ParsedDocument` data class itself is a DR-IO concern and is specified in § 5.7.
...
- **Why Claude multimodal as the primary path rather than a dedicated OCR service.** The system already has exactly one external dependency for model calls (Anthropic, locked in § 3.1). Adding a dedicated OCR service would introduce a second cloud dependency, a second set of credentials, a second rate-limit regime, a second billing stream, and a second code path in the audit chain — all to solve one corner of one input type. Claude Sonnet 4.6 accepts images natively through the existing SDK, so the OCR call uses the same Gateway, the same prompt/schema-hash identity, the same audit-event emission, and the same failure semantics as the rest of the system.
...
**Note (v0.14.0 pivot):** earlier drafts of `Settings` carried `audit_bundle_dir`, `api_host`, and `api_port`. These are gone post-pivot: L1 has no custom HTTP server (§ 3.10) and L1 stages do not own a `runs/<run_id>/` directory layout (DR-DAT-07 / DR-DAT-10) — every output is written to a path the CLI caller declares on `--output`. `Settings` is now strictly value-typed runtime configuration, never directory-layout convention.
...
This arrangement is the concrete realisation of the hermetic-task philosophy from § 2.2 on the *configuration* axis: the task declares its inputs (including the names of secrets it requires), and the environment that supplies them is pluggable. The application therefore satisfies both locked architectural commitment #4 — *L1 tasks are strictly hermetic and runnable directly via their own CLI; Validance is the recommended runner via the L3 integration directory, and L1 has zero `validance-sdk` imports* — and the invariant that registered workflows must never carry secret values.
...
1. **L1 — `grading_rubric/` Python package.** Pure Python, packaged via `pyproject.toml` (PEP 621). Sub-packages: `models`, `parsers`, `assess`, `improve`, `output`, `scorer`, `gateway`, `audit`, `orchestrator`, `cli`, `config`. **Zero Validance imports.** A single console-script entry point is exposed: `grading-rubric-cli`. The CLI offers one subcommand per pipeline stage (`ingest`, `parse-inputs`, `assess`, `propose`, `score`, `render`), a thin `run-pipeline` subcommand that walks all six stages sequentially in one process for tests and local development, and a separate `train-scorer` subcommand for the train-button capability of locked architectural commitment #5 (eight subcommands total — see DR-ARC-08 / DR-SCR-06). Each stage subcommand reads its inputs from CLI-arg paths and writes its outputs to CLI-arg paths; no hidden I/O.
...
- **Why no custom HTTP server in the deliverable.** The SPA needs a backend; a backend already exists — it is **Validance's REST API**, the same one every other Validance integration in the parent repo group already uses. Adding a FastAPI server in the deliverable would re-implement, badly, the things Validance's REST API already does well: workflow submission, approval gates, audit chain, retries, run history. The deliverable would either duplicate them or — worse — paper over them with a thin pass-through that the reviewer would still have to mentally reconcile with the "real" Validance REST. Cutting the FastAPI server cuts a whole layer of complexity and a whole layer of contract drift in one move.
- **Why a CLI subcommand per pipeline stage and not a single end-to-end CLI command.** The CLI is the *single-stage inspection surface* (Path A in § 2.1). A reviewer who wants to see what `assess` does on a particular input runs `grading-rubric-cli assess --input ... --output ...` and reads the output JSON. They do not need to run the whole pipeline to verify one stage. The same per-stage CLI commands are also what Validance's task definitions invoke as their `command:` attributes — one CLI surface, two consumers, no parallel implementations. The thin `run-pipeline` subcommand exists for tests and for the developer's local iteration; it is *not* the production orchestrator.
- **Why Docker images for L2 instead of just shipping the Python package.** Docker images make the L2 → L3 boundary clean: Validance's task model is "run this image with these args"; the image is the natural unit. The same image shipping the same CLI is what runs on `docker run` (Path A) and inside Validance (Path B), so the two paths exercise byte-identical code. The image is also the only inspection surface a reviewer needs — they do not need a Python virtualenv on their host to run a single stage.
- **Why `validance/` is a separate top-level directory and not a sub-package of `grading_rubric/`.** It is the **only** place `validance-sdk` is imported. Keeping it physically separate from L1 makes the "L1 has zero Validance imports" property verifiable by `grep -r 'validance' grading_rubric/` returning empty. It also makes "another orchestrator could wrap L1 by writing its own integration directory" a literal claim — `airflow/` or `snakemake/` would slot in next to `validance/` without touching L1.
- **Why a custom Vite + React + shadcn + Tailwind SPA for L4.** Locked in CLAUDE.md commitment #1 and § 3.3. The stack is the standard way the user builds custom SPAs across the parent repo group; the SPA reverse-engineering loop (write functional spec → generate the SPA → write DR-UI from the observed code) is the methodology for filling DR-UI later.
- **Why no `make run` convenience target.** The full demo path is "run `make register` once to register the workflow against the hosted Validance instance, then open the L4 SPA in a browser". There is no single process to start in the deliverable repository; the things that need to be running (Validance API, PostgreSQL, the L2 worker images) are running on the hosted dev VM, not on the reviewer's host. The single-stage inspection path is `docker run <image> grading-rubric-cli <stage> ...` and is documented in the README as a one-liner.

- **What this decision does not commit.** The exact list of L2 image variants, the exact contents of the L3 workflow definition files, the harvester adapter contract, and the CI pipeline are DR-DEP / DR-INT concerns (§ 5.11 / § 5.12). The internal package layout of L1 is DR-ARC (§ 5.1).

### 3.11 Orchestration layer

**Decision.** L1 (`grading_rubric/`) embeds **no orchestration layer and no Validance imports**. Each pipeline stage is a hermetic function with structured inputs and structured outputs (§ 2.2), exposed individually as a `grading-rubric-cli <stage>` subcommand and chained for tests/dev by a thin in-process orchestrator (`run-pipeline`) that knows nothing beyond "call the next stage with the previous stage's output". **Validance is the recommended runner** and the path through which the deliverable's full V-model experience — audit chain, ApprovalGate-mediated review, retry orchestration, provenance harvest, the L4 SPA — is delivered. The hosted Validance instance on the dev VM is the canonical demo target. The L3 integration directory (`validance/`) is the **sole** site in the repository that imports `validance-sdk`; L1 and L2 remain orchestration-agnostic.

This realizes the locked architectural commitment #4 from `CLAUDE.md` § 6: *tasks are strictly hermetic and runnable directly via their own CLI; Validance is the recommended runner and the path through which the full V-model experience is delivered*. It also realizes commitment #1 (the L4 SPA, talking to Validance's REST API) and commitment #7 (HITL realized via Validance's `ApprovalGate`, not custom code).

**Two demo paths from the same L1 task code:**

1. **Path A — direct CLI invocation.** `docker run <image> grading-rubric-cli <stage> --input … --output …` runs one pipeline stage in isolation. Single-stage inspection. No audit chain, no ApprovalGate, no UI. The ExplainedRubricFile (`grading-rubric-cli render`) is the same JSON the Validance path produces; what is missing is the cross-cutting machinery, not the deliverable.
2. **Path B — Validance + L4 SPA.** The L3 directory pre-registers a `grading_rubric.assess_and_improve` workflow on the hosted dev-VM Validance instance. The SPA (L4) calls Validance's REST API to start the workflow, observes ApprovalGate-mediated review at the propose→render boundary, and renders findings, proposed changes, and the `AuditBundle` view harvested from Validance's audit chain. The teacher's accept/reject decisions and any re-measurement iteration loop are gated by ApprovalGate, never autonomous (§ 2.5).

The conditions for Path B's wrap to work are locked by earlier decisions and require **zero changes** to L1 code:
...
- The application has no on-disk cache (§ 3.8), so there is no hidden state competing with Validance's task-level cache.
- Every LLM call is routed through the Gateway (§ 3.2), so the gateway-emitted operation events become the rows that the L3 harvester turns into the typed `AuditBundle` shape (§ 4.8).

A different orchestrator (Snakemake, Airflow, Argo, Prefect, plain bash) could wrap L1 by writing its own integration directory analogous to L3 — translating that engine's task model to L1's CLI subcommands and harvesting whatever provenance the engine emits into the same `AuditBundle` view. The cross-cutting concerns then become **that orchestrator's responsibility**, not L1's. § 5.12 *DR-INT* locks the L3 contract that Validance integration must satisfy; § 5.11 *DR-DEP* locks the deployment artefacts (L1 package, L2 images, L3 integration directory, L4 SPA build).

**Rationale.**

- **Why Validance as the recommended runner.** The deliverable needs an audit chain, an approval flow, retry orchestration, a backend↔frontend link, and provenance — every one of these is a primitive Validance already provides (`Catalog`, `ApprovalGate`, `Pipeline`, `PersistentWorker`, `LearnedPolicy`, REST API, audit emitter). Re-implementing them inside L1 would inflate the task code with infrastructure that has nothing to do with rubric assessment, contradict commitment #2 (LLMs as measurement instruments — the *task* should be tightly scoped), and obscure the engineering signal the reviewer is looking for. Letting Validance own these concerns keeps L1 honest about its job: *measure rubric quality, propose improvements, render the output*.
- **Why keep L1 strictly hermetic and CLI-callable.** Orchestrator-readiness is free because we already paid for it in § 2.2 / § 3.7 / § 3.8. Keeping zero Validance imports in L1 means: (a) the reviewer can `docker run … grading-rubric-cli <stage>` to inspect any stage in isolation without spinning up Validance; (b) tests substitute the gateway and run unit tests with no SDK installed; (c) a different orchestrator could wrap L1 tomorrow without touching task code. The hermetic boundary is the contract that makes both the CLI inspection path and the Validance demo path possible from the same code.
- **Why the hosted dev-VM instance is the canonical demo target.** A self-contained "install Validance locally" path would require the reviewer to install, configure, and learn an unfamiliar engine just to see the deliverable run end-to-end — which is exactly the cost commitment #4 originally tried to avoid before the pivot. Pointing at a live, working Validance instance instead lets the reviewer see the full V-model experience in one click without any local setup, while the CLI path remains as the self-contained alternative for stage-level inspection.
- **What this decision does not commit.** The exact L3 directory layout, the proposal-payload mapping for `ProposedChange`, the harvester contract, and the workflow registration script are DR-INT concerns (§ 5.12). The Docker image set, the make targets, and the L4 SPA build path are DR-DEP concerns (§ 5.11). This § commits only to: *Validance is the recommended runner, the L3 directory is the sole `validance-sdk` import site, L1 stays hermetic*.
...
**Rationale.** `OperationRecord.details` is a discriminated union, so a future ML classifier path or a human-only path slots into the same audit-view shape without schema migration. Each retry is its own `OperationRecord` linked via `retry_of`, so the harvested view shows the full chain (built from Validance's per-attempt task records on Path B) instead of collapsing it into a single ambiguous status. `validation_retry_count` from § 3.2 is therefore a *derived view* (count of retried operations following the chain), not a stored counter on `LlmCallDetails`. `iteration_history` is the complete trajectory of the SR-AS-09 re-measurement loop and is empty for single-pass runs; on Path B each iteration corresponds to a teacher-gated `ApprovalGate` decision (§ 2.5).

**Index / detail split.** `AuditBundle.operations` carries `OperationSummary` entries, not full `OperationRecord` objects. Each summary is the cheap, queryable view (timing, status, digests, kind) plus a `details_path` pointer to a per-operation JSON object holding the full `OperationRecord`. On Path B the harvester resolves each `details_path` against Validance's run storage; the SPA (L4) reads the bundle via Validance's REST API and lazy-loads detail blocks on drill-in. The on-disk realization for the harvested view is locked in § 5.3 *DR-DAT* and § 5.12 *DR-INT* — the L3 harvester is the sole writer, and the index/detail invariant remains a `Must` (DR-DAT-07a). The split keeps the index small enough for the UI to load eagerly while every `LlmCallDetails.raw_responses` payload (which can be large and which is the privacy-sensitive surface) lives in its own block that is loaded only when a reviewer drills into a specific operation. `details_kind` on the summary is the closed `OperationKind` enum so the UI can filter and group by kind without loading any detail block.

**Privacy note.** The deliverable (`ExplainedRubricFile`, § 4.9) **never contains raw student copy text**. The audit-view bundle may contain transcribed student text inside `LlmCallDetails.raw_responses` and inside operations linked from `OcrCallDetails.underlying_operation_id`, because faithful reconstruction of a measurement requires the exact input and output. On Path B the bundle lives inside Validance's run storage on the hosted dev-VM instance; sharing it is the teacher's decision, mediated by Validance's access controls (commitment #4). On Path A no bundle is produced at all — only the per-stage CLI output that the reviewer requested — so the raw-student-text surface does not exist on the inspection path. The deliverable and the audit-view bundle are separate artefacts for exactly this reason: the deliverable is teacher-distributable, the bundle is reviewer-only.
...
| **DR-ARC-07** | Must | **L1 has zero Validance imports.** No file under `grading_rubric/` (the L1 package), nor any Docker image baked from it (L2), may `import validance`, `import validance_sdk`, `import validance_workflow`, or any module thereof; `grading-rubric-cli` and every stage callable run successfully without `validance-sdk` installed. The boundary is verifiable by a single grep over the L1 source tree (`grep -rn 'validance' grading_rubric/` returns zero matches) and by the L1 dependency declaration in `pyproject.toml` (no `validance-sdk`, no `validance-workflow`). The L3 integration directory (`validance/`, § 5.12 *DR-INT*) is the **sole** site in the repository that imports `validance-sdk`. This is the in-code realization of locked commitment #4. | § 2.1, § 3.10, § 3.11, § 5.12 |
| **DR-ARC-08** | Must | The `cli` sub-package exposes a single console-script entry point `grading-rubric-cli` with **one subcommand per pipeline stage**, a thin orchestrator subcommand, and a separate train-scorer subcommand for the train-button capability of DR-SCR-06: `ingest`, `parse-inputs`, `assess`, `propose`, `score`, `render`, `run-pipeline`, and `train-scorer` — **eight subcommands**. The first six are the per-stage subcommands of the assessment pipeline; `run-pipeline` chains them in order via the in-process `orchestrator` (DR-ARC-04) and is the same code path the test suite exercises; `train-scorer` invokes `scorer.train_scorer(...)` (the stub of DR-SCR-05) and is **not** chained into `run-pipeline` because training is a separate capability with a separate Validance workflow (DR-INT-02). Each per-stage subcommand reads its structured input from disk (a JSON / Pydantic-validated file or a directory of inputs declared by the stage's `stage_inputs` schema), invokes the stage callable through the `Stage` protocol, and writes the structured output to a path the user specifies; this is the unit of single-stage inspection on Path A. The CLI is a thin shell — every per-subcommand handler is at most input validation, calling the stage callable, and writing the result. | § 3.10, § 3.11, DR-SCR-06 |
...
| **DR-LLM-08** | Must | Every successful or failed gateway call calls `audit_emitter.record_operation(...)` exactly once with a fully-populated `LlmCallDetails`: `prompt_id`, `prompt_version`, `prompt_hash`, `schema_id`, `schema_hash`, `model`, `temperature`, `samples`, `tokens_in: int`, `tokens_out: int` (totals across samples — the SDK reports these per `messages.create` call and the gateway sums them), `rate_limit_retries`, `raw_responses` (the parsed tool-input blocks for every sample, including ones that failed validation in DR-LLM-04 — kept as `JsonValue` so the audit captures the actual model output that failed). `inputs_digest` is the structured-object digest of `inputs.model_dump(mode="json")`; `outputs_digest` is the structured-object digest of `MeasurementResult.samples` (or `None` on failure). The `audit_emitter` is the stderr-event producer of DR-OBS-01; the gateway never writes to a file directly and never reaches into any `runs/<run_id>/` directory. | § 4.8, DR-ARC-06, DR-DAT-06, DR-OBS-01 |
...
| **DR-PER-02** | Must | Concurrency primitive: **`asyncio` for I/O-bound work**, no explicit thread pool for compute. The Anthropic Python SDK exposes an `AsyncAnthropic` client; the gateway's `LlmBackend` Protocol (DR-LLM-09) is async at the seam (`async def create_message(...)`), so every LLM call inside a stage is an `await` and many calls in flight share a single event loop. Stage callables are themselves `async def` and the in-process orchestrator (DR-ARC-04) drives them with `asyncio.run(...)`; per-stage CLI subcommands (DR-ARC-08) wrap the async stage entry point with the same `asyncio.run(...)`. L1 code does **not** instantiate any `concurrent.futures.ThreadPoolExecutor`, does **not** use `multiprocessing`, and does **not** spawn any `threading.Thread`. The **single, narrow exception** is `asyncio.to_thread(...)` used by DR-PER-05 to run synchronous third-party parser libraries (`pypdf`, `pdfplumber`, `python-docx`) without blocking the event loop; that call delegates to `asyncio`'s default executor under the hood (a thread pool L1 does not own, configure, or hold a reference to) and is justified because the wrapped libraries are blocking I/O calls into file parsers, not CPU-bound compute. No other code path in L1 uses threads, by intent. | § 2.2, § 3.9, DR-PER-05 |
| **DR-PER-03** | Must | Parallelism boundaries: within a single stage call, two work-units run concurrently — (a) **multi-sample LLM calls** for one measurement (per DR-LLM-05, the gateway issues `samples` independent calls — these run via `asyncio.gather` so the wall-clock cost of `samples=5` is one call, not five) and (b) **per-student-copy fan-out** in the `parse_inputs` and `assess` stages, where each student copy is processed independently and the stage uses `asyncio.gather` over the per-copy coroutines. Stages themselves run **sequentially** — `assess` waits for `parse_inputs` to finish before it starts, regardless of orchestrator — because each stage's outputs feed the next. The orchestration layer (in-process `orchestrator` on Path A; Validance on Path B) enforces this stage-order property; no L1 stage attempts to start work in another stage. | § 3.9, SR-PRF-01 |
| **DR-PER-04** | Must | Gateway call-rate bound: a **single semaphore inside the gateway** bounds the number of in-flight LLM calls per process to `Settings.llm_max_concurrency` (default `8`). Every `gateway.measure(...)` call acquires the semaphore for the duration of one underlying SDK call (per sample, per validation-retry attempt). This semaphore is the **only place in L1 that bounds the LLM call rate** — stages do not maintain their own LLM-rate semaphores, the parsers do not bound OCR fan-out separately at the call-rate axis, and the orchestrator does not bound stage parallelism (because stages do not run in parallel — DR-PER-03). The stage-local fan-out semaphores of DR-PER-06 are a *different* axis (in-memory batch-size bound, not call-rate bound) and do not contradict this rule. A reviewer auditing the call-rate surface looks at exactly one place. | § 3.9, SR-PRF-01 |
| **DR-PER-05** | Must | Per-call timeout reuse: every LLM call is bounded by `Settings.llm_call_timeout_seconds` (default `60`) per DR-LLM-07. Per-OCR-page calls are LLM calls and inherit the same bound. Per-file parser calls (`pypdf` extraction, `pdfplumber` extraction, `python-docx` parsing) are bounded by `Settings.parser_call_timeout_seconds` (default `30`); the parser timeout is enforced via `asyncio.wait_for` around the synchronous library call run in a thread via `asyncio.to_thread` (the **single** carve-out to DR-PER-02's "no thread pool" rule, justified because the third-party libraries are synchronous and refusing to use a worker thread there would block the event loop). Timeouts are recorded as `FAILED` operation events with `error.code == "TIMEOUT"` per DR-LLM-07's analogue. | § 3.9, DR-LLM-07 |
| **DR-PER-06** | Must | Stage-local memory fan-out bound: `parse_inputs` and `assess` process student copies in chunks of `Settings.student_copy_chunk_size` (default `16`) using a stage-local bounded `asyncio.Semaphore`, so a run with 100 student copies (SR-PRF-01) holds at most 16 `TranscribedPage` lists in memory at any moment. Each chunk's results are folded into the stage's output incrementally, then the chunk is released for garbage collection. **This is a different axis from DR-PER-04** and the two coexist by design: the gateway semaphore is the **call-rate bound** (a process-wide cap on in-flight LLM calls), and the chunk semaphore is the **memory fan-out bound** (a stage-local cap on the number of in-flight per-copy coroutines). The stage-local semaphore never bounds LLM call concurrency — that is exclusively DR-PER-04's job — and the gateway semaphore never bounds in-memory batch size. The two limits compose: the chunk size bounds how many copies are *in flight* in the stage at once, the gateway concurrency bounds how many of their underlying LLM calls run *simultaneously*. | SR-PRF-01 |
...
Defines the **`Scorer` interface** (the abstraction that any future trained model would implement) and the stub `train_scorer` task (the build-time wiring that makes the *train button* real even though no actual model is trained for this delivery). This group is the design-layer realization of locked architectural commitment #5: the hiring team is a tools-delivery group, and what they evaluate is whether the candidate can build a system that has a train button and a replaceable scoring backend — not whether a model has actually been fit. The `scorer` sub-package (DR-ARC-01) is the home of this group; the `score` CLI subcommand (DR-ARC-08) and the `score` Validance task (DR-INT-02) invoke the **runtime scoring** half; the `train-scorer` CLI subcommand (DR-SCR-06) and the separate Validance workflow `grading_rubric.train_scorer` invoke the **train-button** half.

**V-shape carve-out.** DR-SCR is the **only** group in § 5 whose DRs trace primarily to a *locked architectural commitment* (CLAUDE.md § 6 #5) rather than to a System Requirement, because the train-button capability is an architectural framing of the deliverable rather than a user-elicited need: the v1 deliverable does not actually train a model, no User Need was elicited around training, and inventing an SR after the fact would be dishonest about the chain. The runtime scoring DRs (DR-SCR-01 / DR-SCR-02 / DR-SCR-04) still trace to the assessment SRs they participate in (SR-AS-01..03 via the *quality scores* surface, SR-OUT-03 via the explanation organization). The train-button DRs (DR-SCR-03 / DR-SCR-05 / DR-SCR-06 / DR-SCR-07) trace to commitment #5 explicitly and are exempt from the universal *every DR traces to at least one SR* rule of § 6; § 6 documents the exemption when the traceability table lands.

| ID | Criticality | Statement | Trace |
|---|---|---|---|
| **DR-SCR-01** | Must | The `scorer` sub-package exposes a `Scorer` Protocol with one method: `score_rubric(rubric: Rubric, evidence: ScoringEvidence, *, settings: Settings, audit_emitter: AuditEmitter) -> ScoringResult`. `ScoringEvidence` carries the `parse_inputs` outputs (parsed exam question, parsed teaching material, parsed student copies — possibly empty) and the assessment findings produced by `assess`. `ScoringResult` carries `quality_scores: list[CriterionScore]` (one per quality criterion, per § 4.5), `scorer_id: str` (the identifier of the concrete implementation that produced the scores), and `scorer_version: str`. The Protocol is the *sole* surface the rest of the code knows about for scoring; no caller imports a concrete `Scorer` implementation. | § 2.4, § 4.5, SR-AS-09 |
| **DR-SCR-02** | Must | The default implementation `LlmPanelScorer` realizes `Scorer` by delegating to `gateway.measure(...)` against a `score_rubric_panel` prompt with `samples = Settings.scorer_panel_size` (default `5`), aggregating the per-sample scores into `CriterionScore.score` (mean across samples) and setting `CriterionScore.method = QualityMethod.LLM_PANEL_AGREEMENT` (the closed-enum value of § 4.9 reserved for this scorer). **Enum-population scope:** the scorer populates `CriterionScore.method` with **only** `LLM_PANEL_AGREEMENT`; the other `QualityMethod` values (`PAIRWISE_CONSISTENCY`, `SCORE_DISTRIBUTION_SEPARATION`, `SYNTHETIC_COVERAGE`) are populated by the `assess` stage's measurements on the `Measurement.method` field (a different field on a different shape — see § 4.5) and **never** appear on `CriterionScore.method`. The `QualityMethod` enum is shared between the two shapes deliberately so the audit-view bundle and the deliverable can render *what method produced this number* in one consistent vocabulary; the partition of the enum across the two fields is by stage, not by criterion. The **per-sample distribution** is **not** stored on `CriterionScore` — that field is a closed `QualityMethod` enum (§ 4.9) and cannot hold a list of floats — it lives instead inside the `LlmCallDetails.raw_responses` of the `operation` event the gateway emits per DR-LLM-08, where every individual sample is recorded as a `JsonValue`. **Audit join:** `LlmPanelScorer` populates `CriterionScore.source_operation_id` with the `OperationId` of the operation event it produced; the per-sample distribution is then unambiguously joined to the score by that ID — no `(scorer_id, run_id)` lookup, no iteration ambiguity in the SR-AS-09 re-measurement loop. A reviewer who needs the spread reads `audit_bundle.operations[i]` whose `id == criterion_score.source_operation_id` and dereferences its `details_path` per DR-DAT-07a. `LlmPanelScorer` is the only `Scorer` shipped in the v1 deliverable: it produces honest quality scores without requiring any trained model and is the same code path the `score` CLI subcommand invokes on Path A and the `score` Validance task invokes on Path B. Its `scorer_id` is `"llm_panel.v1"`. | § 4.5, § 4.9, DR-LLM-01, DR-LLM-05, DR-LLM-08, DR-DAT-07a, SR-AS-01, SR-AS-02, SR-AS-03 |
| **DR-SCR-03** | Must | An alternative implementation `TrainedModelScorer` is **declared but not shipped trained**: the class exists in the `scorer` sub-package, implements the `Scorer` Protocol, takes a model artefact path on construction, and contains the inference code that *would* call a trained model. In the v1 deliverable its constructor raises `ScorerArtefactMissingError` with a clear message ("no trained scorer artefact present at `<path>` — run `grading-rubric-cli train-scorer` to produce one") so a reviewer who selects it via `Settings.scorer_backend = "trained"` sees the *exact* failure mode the train button is designed to resolve. Its `scorer_id` is `"trained_model.v0"`. The class exists so a future contributor adds *only* the model artefact and the actual inference body, never the wiring. | commitment #5 |
| **DR-SCR-04** | Must | Scorer selection is via `Settings.scorer_backend` (default `"llm_panel"`); valid values are exactly the registered `scorer_id` prefixes (`"llm_panel"`, `"trained_model"`). The `score` stage instantiates exactly one `Scorer` per call from this setting and never holds a reference across calls. Selection at the CLI is via `--scorer-backend <id>` on `grading-rubric-cli score`; selection at the Validance task is via the task's parameters (DR-INT-02). | § 3.7 |
| **DR-SCR-05** | Must | The `scorer` sub-package exposes a `train_scorer(training_evidence: TrainingEvidence, *, settings: Settings, audit_emitter: AuditEmitter) -> TrainedScorerArtefact` callable as the **train-button capability**. `TrainingEvidence` is the same shape as `ScoringEvidence` plus a `ground_truth_grades: list[GroundTruthGrade]` field. `TrainedScorerArtefact` carries `artefact_path: Path`, `training_run_id: RunId`, `scorer_id: str`, and a `metrics: dict[str, float]` summary. In the v1 deliverable `train_scorer` is a **stub**: it validates its inputs against the schemas, emits one `operation` event with `details_kind="ml_inference"` and `status="skipped"` whose `error.code == "STUB_NOT_TRAINED"` and a human-readable message explaining the capability framing per commitment #5, and writes a placeholder artefact file containing the validated inputs hash and the stub marker. The function does **not** call `gateway.measure(...)`, does **not** train any model, and does **not** consume LLM tokens. | commitment #5, DR-OBS-01 |
| **DR-SCR-06** | Must | The train-button capability is exposed as a CLI subcommand `grading-rubric-cli train-scorer --input <training_evidence.json> --output <artefact_path>` (an addition to the seven subcommands of DR-ARC-08, taking the count to **eight**). On Path B, `train_scorer` is registered as a separate Validance workflow `grading_rubric.train_scorer` — **not** part of the `assess_and_improve` workflow of DR-INT-02 — so a reviewer can trigger it from the L4 SPA via Validance's standard workflow-run UI without entering the assessment loop. The L4 SPA renders a *Train scorer* button that submits this workflow with the current run's evidence as input; the SPA shows the same stub `STUB_NOT_TRAINED` outcome that the CLI subcommand surfaces, so the capability is *visibly real* even though no model is actually trained. | DR-ARC-08, DR-INT-02, commitment #5 |
| **DR-SCR-07** | Should | Replaceability invariant: a future trained model is added to the deliverable by (a) producing an artefact file at the path `Settings.trained_scorer_artefact_path`, (b) replacing the body of `TrainedModelScorer.score_rubric` with the real inference call, and (c) replacing the body of `train_scorer` with the real training loop. **No other file in the L1 package needs to change**, no caller of `Scorer.score_rubric` needs to know which backend is in use, and no test outside the `scorer` sub-package needs to be re-run. This invariant is the structural property that makes the train-button capability honest: the wiring is real, only the model is absent. A reviewer can verify the invariant by reading the `scorer` sub-package and running `grep -rn 'LlmPanelScorer\|TrainedModelScorer' grading_rubric/` — the only references outside `grading_rubric/scorer/` should be in the `Scorer` Protocol re-export from `grading_rubric.__init__`. | commitment #5, DR-ARC-12 |
...
| **DR-DEP-04** | Must | L4 is a separate buildable: `cd frontend && npm install && npm run build` produces a static SPA bundle that talks to Validance's REST API at the URL specified in `frontend/.env.production` (default: the hosted dev-VM Validance instance, per commitment #4). The L4 build does not import any L1 Python code, does not read L1 source files, and never runs in the same process as any L1 task. The wire types the SPA consumes are generated from L1's Pydantic models via the JSON-Schema codegen path of DR-DAT-03 / DR-DAT-04, packaged into `frontend/src/generated/`. | § 3.3, § 3.10, DR-ARC-10 |
...
| **DR-DEP-06** | Must | The deliverable supports two demo paths from the same L1 task code: **Path A** — `docker run <image> grading-rubric-cli <stage> --input … --output …` runs one pipeline stage in isolation, against any image built by `make images`, with no Validance involvement; **Path B** — after `make register` against the hosted Validance instance, the L4 SPA (started via `make dev` for local iteration or served from any static host for the demo) calls Validance's REST API to start the `grading_rubric.assess_and_improve` workflow and renders the run, the ApprovalGate-mediated review, and the harvested `AuditBundle` view. **The `grading_rubric.train_scorer` workflow registered alongside it (DR-INT-02 / DR-SCR-06) is a separate capability outside the main demo path** — the reviewer triggers it from the L4 SPA's *Train scorer* button to see the train-button capability surface, but it is not part of the assess-and-improve loop and is not exercised by the README's two-snippet quick-start. The README documents both demo paths and shows the exact commands; the reviewer is expected to know which path they want, not to discover it. | § 3.11, DR-ARC-08, § 5.12, DR-SCR-06 |
...
| **DR-INT-02** | Must | `register.py` registers **two** Validance workflows: (i) `grading_rubric.assess_and_improve` — the assessment pipeline, with **one Validance task per L1 pipeline stage**: `ingest` → `parse_inputs` → `assess` → `propose` → `approve` (an `ApprovalGate`, see DR-INT-06) → `score` → `render`; and (ii) `grading_rubric.train_scorer` — the **separate** train-button workflow per DR-SCR-06, with a single task wrapping the `train-scorer` CLI subcommand (the stub of DR-SCR-05 in v1), explicitly **not** chained into `assess_and_improve` because training is an independent capability with a different input contract (`TrainingEvidence` rather than `IngestInputs`). Each L1 stage / capability is wrapped as a Validance task whose execution is `docker run <image> grading-rubric-cli <subcommand> --input … --output …` with the `<image>` from L2 (DR-DEP-03) and the input/output paths declared on the task's `inputs` / `outputs` schema. The Validance workflow definitions do **not** call any L1 Python function in-process; the only L1↔Validance contact surface is the CLI subcommand exit code, the structured `--output` file, and the structured stderr operation events that the harvester consumes. | § 3.11, DR-ARC-08, DR-DEP-06, DR-SCR-06 |
| **DR-INT-03** | Must | Every L1 stage that calls the LLM (per DR-LLM-01) emits its operation events via the `audit_emitter` to `stderr` as structured-JSON lines matching the wire schema locked by DR-OBS-01 (and committed at `schemas/audit_event.v1.schema.json` per DR-DAT-03) when invoked through Validance. The L3 task wrapper captures this stderr stream as part of Validance's standard task-output capture; the harvester (DR-INT-05) joins it with Validance's per-task audit-chain rows to build the typed `AuditBundle` view. No private file format crosses the L1↔L3 boundary; the contract is *structured stderr lines (validated against `audit_event.v1.schema.json`) + Validance's own audit chain*. | § 4.8, DR-ARC-06, DR-OBS-01, DR-DAT-03 |
...
**Carve-out — DR-SCR train-button capability.** § 5.10 *DR-SCR* contains the only DRs in this design that do not all trace to a System Requirement. The runtime scoring DRs (DR-SCR-01 / DR-SCR-02 / DR-SCR-04) trace normally — they participate in the assessment SRs they implement (SR-AS-01..03, SR-OUT-03). The **train-button DRs** (DR-SCR-03 / DR-SCR-05 / DR-SCR-06 / DR-SCR-07) trace instead to **CLAUDE.md § 6 architectural commitment #5** — *"Train button is a capability, not an output"* — and are explicitly **exempt** from the universal "every DR traces to at least one SR" rule above. The carve-out is documented in the intro paragraph of § 5.10 and is the only such exemption in this document; any future DR that does not trace to an SR must be justified the same way (named locked commitment, exemption noted both in the relevant § 5 group intro and here).
...
|---|---|---|---|
| 0.16.1 | 2026-04-11 | Wiktor Lisowski | **Documentation hygiene patch — drop the development-tool name from the body of the design document.** Per the convention recorded in `CLAUDE.md` § 4 *Vocabulary discipline*, the SPA generation tool used during development of the L4 frontend (the same way the Cursor IDE is used during development of every layer) is **not** part of the product architecture and must not appear in official documentation under `docs/`. The L4 SPA is described by its **technology stack** (Vite + React + shadcn + Tailwind, per § 3.3) and by its **architectural role** (custom SPA in `frontend/`, talking to Validance's REST API), nothing else. Edits in this patch: status block on line 5 reworded; § 1.2 *Scope* bullet, § 2.1 *system diagram* (the ASCII Path A / Path B picture and the surrounding paragraphs), § 2.5 *human-in-the-loop*, § 3 decision register row #10, § 3.10 rationale block (the *"Why … for L4"* bullet rewritten as *"Why a custom Vite + React + shadcn + Tailwind SPA for L4"* and the `make run` bullet's "open the SPA in a browser" wording neutralized), § 3.11 *Orchestration layer* (Path B framing + commitment-#1 quote), DR-ARC-01 (the L4 deployable description in the Statement column), § 5.11 *DR-DEP* group intro and DR-DEP-01 Statement column, and the v0.14.0 modlog entry's two literal mentions (the historical record is preserved by neutralizing the wording rather than rewriting the entry's intent — the entry still describes the v0.14.0 architectural shift, just without naming the SPA generation tool). No semantic change to any DR, no model change in § 4, no new SR or UR; this is a **PATCH** bump (`0.16.0` → `0.16.1`) per the v0.7.1 §1 versioning convention because the change is purely a naming / hygiene clarification. Three review passes from the v0.16.0 round are unchanged. |
| 0.16.0 | 2026-04-11 | Wiktor Lisowski | **R4 cluster (DR-IO + DR-PER + DR-SCR) folded in — 23 new Design Requirements — and three back-to-back Codex review passes absorbed (post-pivot stale content + R4-on-R4 fixes + R4 polish).** **R4 fold-in.** § 5.6 *DR-IO* (8 DRs) locks parser/OCR with a role-tagged ingest shape, the closed `SourceFormat` enum extended for `INLINE_TEXT` (covering SR-IN-05 *paste a starting rubric* alongside file-upload and absent), and a **role-aware no-text-layer PDF policy**: a no-text-layer PDF in the *student-copy* role is routed through the `StudentCopyReader` Protocol (the OCR seam, DR-ARC-05) while the same shape in the *exam question*, *teaching material*, or *starting rubric* role is a hard `ParseError` — those inputs are typed-text artefacts the teacher always has in machine-readable form. `IngestInputs` carries `exam_question: ExamQuestionSource`, `teaching_material: list[TeachingMaterialSource]`, `starting_rubric: StartingRubricSource \| None` with each source variant being either `{path: Path}` or `{inline_text: str}` so the three SR-IN-05 forms (file / paste / absent) are explicit in the model. § 5.9 *DR-PER* (8 DRs) locks the concurrency / back-pressure / timeout surface with two **distinct, composable** axes: DR-PER-04 the **gateway call-rate bound** (the only place that bounds LLM call rate) and DR-PER-06 the **stage-local memory fan-out bound** (per-stage parallel-await semaphore protecting against unbounded fan-out into LLM samples or per-copy OCR work) — the two limits are deliberately on different axes (rate vs concurrency) and compose without contradiction. DR-PER-07 *progress signal* **promoted from Should to Must** to align with SR-PRF-02's >5s heartbeat requirement, with an explicit Path A / Path B contract: Path B SPA polls Validance every ≤2s via TanStack Query against `GET /api/runs/{run_id}/state` and renders the latest `stage.start` / `operation` / `stage.end` event timestamps as the heartbeat; Path A operator runs the CLI in a foreground shell where the running process is itself the visible signal. DR-PER-08 *cancellation* weakened so the L1 task code merely **terminates non-zero** on orchestrator-signalled `SIGTERM` — no custom signal handler is registered, default Python `SIGTERM` semantics apply, no automatic translation into `CancelledError` is claimed; cleanup is best-effort and the audit chain is reconstructed by L3 from the partial stderr stream. § 5.10 *DR-SCR* (7 DRs) locks the **train-button capability** per CLAUDE.md § 6 commitment #5: the `Scorer` Protocol (DR-SCR-01), the LLM-panel scorer (DR-SCR-02), the trained-model scorer stub (DR-SCR-03), backend selection via `Settings.scorer_backend` (DR-SCR-04), the `train_scorer` stage stub (DR-SCR-05), the eighth `grading-rubric-cli train-scorer` subcommand (DR-SCR-06), and the separate `grading_rubric.train_scorer` Validance workflow (DR-SCR-07). DR-SCR-02 was specifically rewritten to fix a model-drift bug surfaced in review: the per-sample score distribution is **not** stored on `CriterionScore.method` (a closed `QualityMethod` enum, § 4.9, which cannot hold a list of floats); the aggregated mean lands on `CriterionScore.score`, the method enum is set to `QualityMethod.LLM_PANEL_AGREEMENT`, and the per-sample distribution lives inside the `LlmCallDetails.raw_responses` of the `operation` event the gateway emits per DR-LLM-08 (where it can be inspected for the SR-AS-09 re-measurement loop without violating the closed enum invariant). **DR-SCR carve-out** — the train-button DRs (DR-SCR-03, DR-SCR-05, DR-SCR-06, DR-SCR-07) trace to CLAUDE.md § 6 architectural commitment #5 *"Train button is a capability, not an output"* rather than to a System Requirement; this is the only such exemption in the document and is documented both in the § 5.10 group intro and in § 6 immediately after the universal "every DR traces to at least one SR" rule. **Codex review pass 1 (post-pivot stale content).** Three High-severity stale-content fixes: (i) § 2.3 dropped the "content-hashed caching of identical calls" reference (caching was rejected in v0.10.0 § 3.8); (ii) § 3.4 *cache reference* rewritten to point at decision #8 with the explicit answer "no application-level cache"; (iii) § 3.5 OCR seam rewritten to drop "content-hash caching" and frame the seam around the actual L1 traits — *prompt/schema hashing*, *structured audit-event emission*, *retry-once validation*. Five Medium-severity drift fixes: (iv) § 3.3 *SPA framing* rewritten to drop the obsolete Python HTTP server framing — the SPA reads `VITE_VALIDANCE_BASE_URL` and talks to **Validance's REST API as its backend** with no custom L1 HTTP server in the deliverable; (v) § 3.7 *locked-commitment quote* updated to the post-pivot wording (*L1 tasks are strictly hermetic and runnable directly via their own CLI; Validance is the recommended runner via the L3 integration directory, and L1 has zero validance-sdk imports*); (vi) § 4 intro `backend/grading_rubric/models/` → `grading_rubric/models/`; (vii) § 4.1 *three-case hash rule* expanded inline citing DR-DAT-06; (viii) DR-DAT intro rewritten to drop the "writes the audit bundle and the explained rubric file to disk under runs/<run_id>/" framing inherited from the pre-pivot draft, replaced with the post-pivot strictly-per-stage persistence model where `AuditBundle` is a typed *view* the L3 harvester constructs from Validance's audit chain on demand. One Low: dropped the invalid `SR-NF-04` and `SR-NF-02` trace IDs from DR-ARC-01 / DR-ARC-03 / DR-ARC-05 / DR-DAT-02 (those SR-NF-* IDs do not exist — `requirements.md` § 5 has SR-PRF, SR-OBS, etc. but no SR-NF cluster). DR-INT-04 § 4.7 → § 4.6 (the `ProposedChange` model lives in § 4.6, not § 4.7). DR-INT-06 *HITL* rewritten to make the polling-not-webhook framing explicit: the SPA polls Validance's REST API every ~2s via TanStack Query against `GET /api/runs/{run_id}/state` while the gate is open, never receives inbound webhooks (browsers cannot host webhook endpoints). **Codex review pass 2 (R4-on-R4 fixes).** Three High: (i) the DR-SCR carve-out documented above; (ii) the DR-SCR-02 `CriterionScore.method` enum-violation fix documented above; (iii) DR-PER-07 promoted from Should to Must to align with SR-PRF-02. Five Medium: (iv) DR-IO inline-text starting-rubric coverage (SR-IN-05) added per above; (v) the role-aware no-text-PDF policy added per above; (vi) `EvidenceProfile.synthetic_responses_used: bool = False` field added in § 4.4 so SR-AS-06 *synthetic responses* has a model field to land on (set by `assess` when no real student copies are available); (vii) the DR-PER-04 / DR-PER-06 concurrency-bounds wording rewritten to make the two-axes composition explicit and remove the apparent self-contradiction; (viii) DR-PER-08 `SIGTERM` overclaim weakened per above. One Medium: train-scorer subcommand propagated from DR-SCR-06 into DR-ARC-08 (eight subcommands total), § 3.10 (the CLI subcommand list mentions `train-scorer` and references DR-ARC-08 / DR-SCR-06), and DR-INT-02 (which now registers **two** Validance workflows — `grading_rubric.assess_and_improve` and `grading_rubric.train_scorer`). One small additive: `LlmCallDetails.rate_limit_retries: int = 0` field added in § 4.8 so DR-LLM-07's *transparent transport-level 429 retries* have a place to record their count without crossing the operation boundary as separate events. The R4-Low metadata staleness (line 5 status block) is absorbed by this version bump itself. **Codex review pass 3 (R4 polish, absorbed in the same v0.16.0 bump).** A third review surfaced one High and several Medium / Low findings, all internal-consistency drift between R4 additions and adjacent un-touched sections — the philosophy is intact, only the propagation needed to land. **Codex pass 3 High — `InputProvenance` cannot represent inline text.** Symmetric twin of the `synthetic_responses_used` fix from pass 2: the v0.12.1 `InputProvenance` shape carried `*_path` / `*_hash` parallel-list fields and could not represent the SR-IN-05 *pasted-text starting rubric* form. Replaced with a role-tagged `InputSource` discriminated record (`kind: FILE | INLINE_TEXT` plus `path | marker | hash`) and four role-tagged fields (`exam_question`, `teaching_material`, `starting_rubric`, `student_copies`) so that file-uploaded inputs and pasted-text inputs are first-class on the audit-view bundle. New `InputProvenance shape — files vs inline text` rationale paragraph added to § 4.4 explaining the discriminator and the marker discipline (the marker identifies *which inline slot* the input filled, never derived from the inline content). **Reviewer #1 High — DR-PER-02 vs DR-PER-05 literal contradiction.** DR-PER-02 used to say "no `concurrent.futures.ThreadPoolExecutor`" while DR-PER-05 introduced `asyncio.to_thread` (which delegates to `asyncio`'s default thread executor under the hood) as the carve-out for synchronous parser libraries. Softened DR-PER-02 to "no *explicit* thread pool" and explicitly named `asyncio.to_thread` as the single, narrow exception with the justification (the wrapped libraries are blocking I/O parsers, not CPU-bound compute), then cross-referenced DR-PER-05 in the Trace column. **Codex pass 3 Medium — DR-SCR-02 audit join underspecified across iterations.** `(scorer_id, run_id)` was insufficient to identify the right operation when the SR-AS-09 re-measurement loop produces multiple `score` operations per run. Added `source_operation_id: OperationId | None = None` field to `CriterionScore` (§ 4.9) — the unambiguous join key — and rewrote the DR-SCR-02 audit-join sentence to reference it, dereferencing through the DR-DAT-07a index/detail invariant. **Codex pass 3 Medium — DR-IO-03 reopens routing ambiguity.** DR-IO-03 listed image inputs as routed to `StudentCopyReader` without repeating the role restriction DR-IO-02 had locked. Tightened DR-IO-03 to "**only when the input's declared role from `ingest` is `student_copy`**" with the same image formats in the exam/teaching/rubric roles raised as a hard `ParseError`, since those inputs are typed-text artefacts the teacher always has in machine-readable form. **Codex pass 3 Medium — Path A vs SR-PRF-02 teacher-facing scope.** DR-PER-07's Path A clause read as if it claimed Path A satisfied SR-PRF-02 itself; SR-PRF-02 is teacher-facing and Path A is reviewer/developer inspection. Added an explicit scope-out: SR-PRF-02 is teacher-facing by construction and is satisfied by Path B; Path A is **out of the SR-PRF-02 teacher-facing interpretation**, with developer-grade live progress available on Path A by piping `stderr` through `jq` (the `--emit-operations` flag of DR-OBS-04 makes this trivial). **Reviewer #1 Medium — stage-local model home.** DR-IO-07's `IngestInputs` / `ExamQuestionSource` / etc. are stage-local input shapes that should not bloat `grading_rubric.models` (which is the codegen surface). DR-DAT-01 expanded with the rule: a shape lives in `grading_rubric.models` if **and only if** it appears in § 4; everything else is stage-local and lives next to its stage (`grading_rubric.parsers.models`, `grading_rubric.scorer.models`); stage-local classes still inherit DR-DAT-02 strictness but are deliberately out of the DR-DAT-04 codegen surface because the SPA never sees them. **Reviewer #1 Medium — DR-SCR-02 enum-population scope.** Added a one-paragraph note to DR-SCR-02 that the scorer populates `CriterionScore.method` with **only** `LLM_PANEL_AGREEMENT`; the other `QualityMethod` values (`PAIRWISE_CONSISTENCY`, `SCORE_DISTRIBUTION_SEPARATION`, `SYNTHETIC_COVERAGE`) are populated by the `assess` stage's measurements on `Measurement.method` (§ 4.5) — a different field on a different shape — so the partition of the shared enum is by stage, not by criterion. **Codex pass 3 Low — DR-IO-06 framing.** Updated "iterates the file list from `ingest`" to "iterates the **input records** from `ingest`" (each carrying either a file path or an inline-text body) so the inline-text branch is not invisible in the partial-failure machinery. The `ParseError` source identifier is similarly framed as "file path for the file branch, the inline marker for the inline-text branch". **Codex pass 3 Low — DR-DEP-06 train-scorer carve-out.** Added a sentence: `grading_rubric.train_scorer` is a **separate capability outside the main demo path**, triggered from the L4 SPA's *Train scorer* button per DR-INT-02 / DR-SCR-06, not part of the assess-and-improve quick-start the README opens with. § 6 traceability sweep still deferred to the round that closes out the remaining DR-AS / DR-IM / DR-UI groups. |
| 0.15.0 | 2026-04-11 | Wiktor Lisowski | **R3 cluster folded in post-pivot: § 5.2 *DR-LLM* (11 DRs) and § 5.8 *DR-OBS* (4 DRs) — 15 new Design Requirements.** R3 was originally drafted pre-pivot in `notes/dr-r3-draft.md` (DR-LLM + DR-OBS together because both bind to the gateway↔audit seam); the pivot deletes large parts of the original DR-OBS draft because Validance now owns the audit chain, the on-disk bundle layout, the streaming surface, and the read API. Reconciliation log: DR-LLM is mostly intact (the gateway is L1, untouched by the pivot) — surgical edits only to DR-LLM-04 (`retry_of` chains reconstructed by L3 from the stderr stream, not by an in-process audit writer), DR-LLM-08 (the `audit_emitter` is L1's stderr-event producer per DR-OBS-01, not an in-process bundle writer; `tokens_in` / `tokens_out` are flat totals across samples), DR-LLM-11 (promoted to Must, framed as the L1↔L3 audit contract per DR-INT-03). DR-LLM-12 (`gateway.dry_run` debug helper) dropped as premature. DR-LLM-07 *transparent rate-limit retries* rule locked: transport (HTTP 429) retries are wire-level transparent, validation retries (DR-LLM-04) cross the operation boundary as separate linked events. DR-OBS shrinks dramatically: pre-pivot draft was 10 DRs, post-pivot fill is 4 DRs because Validance owns the audit chain. **DR-OBS-01** locks the structured-JSON wire format on `stderr` and the closed five-event-kind set `{run.start, stage.start, operation, stage.end, run.end}`, plus the **producer rule**: `run.*` events are emitted **only** by the in-process orchestrator of DR-ARC-04 (which always knows the full pipeline context); per-stage CLI invocations and Validance task wrappers emit only `stage.start` / `operation` / `stage.end` and the harvester (DR-INT-05) synthesizes the `run.*` envelope itself when materializing the `AuditBundle` view. **DR-OBS-02** locks the privacy boundary on `raw_responses` (transcribed text only inside operation `details`, never on stage/run envelope events, never in the deliverable file or SPA index views). **DR-OBS-03** locks the `AuditEmitter` interface methods (`start_run` / `start_stage` / `record_operation` / `end_stage` / `end_run`), with `start_run` / `end_run` no-ops on per-stage and Validance task emitters so the producer-rule cannot be violated by accident. **DR-OBS-04 (Should)** keeps the `audit.snapshot()` debug helper used by tests and by the `--emit-operations` CLI flag. **§ 5.3 *DR-DAT-03* surgically updated** to also regenerate `schemas/audit_event.v1.schema.json` (the L1↔L3 wire schema) so the harvester can validate every line it reads against the same committed schema L1 emits to. **§ 5.12 *DR-INT-03* surgically updated** to reference DR-OBS-01 by ID for the wire-schema contract, replacing the prose "structured-JSON stderr operation events" with the explicit `audit_event.v1.schema.json` artefact name. § 1 status block updated to reflect 6 of 12 DR groups filled (≈ 50 DRs total). § 6 traceability sweep deferred to the round that closes out the remaining DR-AS / DR-IM / DR-UI / DR-IO / DR-PER / DR-SCR groups. The R3 draft `notes/dr-r3-draft.md` remains as the historical record of the post-pivot reconciliation and will be archived (or deleted) once the next DR cluster lands. |
| 0.14.0 | 2026-04-11 | Wiktor Lisowski | **Validance pivot folded in (per `notes/validance-pivot.md`).** The single biggest design-document change since the v0.13.0 fill: tasks remain strictly hermetic and CLI-callable, but Validance is now *the* recommended runner and the path through which the deliverable's full V-model experience is delivered, with the hosted Validance instance on the dev VM as the canonical demo target. The deliverable is reframed as a four-layer architecture: **L1** the pure Python package `grading_rubric/` with one `grading-rubric-cli` console-script subcommand per pipeline stage and **zero Validance imports**; **L2** Docker images that bake L1 + the CLI as their entrypoint, knowing nothing about Validance; **L3** a separate top-level `validance/` directory containing workflow definitions, an idempotent registration script, a proposal-payload mapping for `ProposedChange`, and an audit-chain harvester — the **only** site in the repository that imports `validance-sdk`; **L4** a custom SPA in `frontend/` (Vite + React + shadcn + Tailwind) talking to Validance's REST API. EPFL evaluates by either (a) running individual CLI commands directly (`docker run <image> grading-rubric-cli <stage> --input … --output …`) for single-stage inspection or (b) calling Validance's REST API (or using the SPA on top of it) for the full pre-registered workflow with audit chain, ApprovalGate-mediated review, retries, and provenance. Audit chain, provenance, retry orchestration, the human-approval flow, and the backend↔frontend link are **delivered by Validance** when running the L3 path and are deliberately **not re-implemented** inside L1 task code. **§ 2 changes.** § 2.1 system diagram rewritten as a four-layer ASCII picture showing both Path A (CLI direct) and Path B (Validance + SPA) and the L1↔L2↔L3↔L4 boundaries; § 2.2 *hermetic-task philosophy* rewritten around three execution surfaces (direct CLI invocation, in-process stage chain via the thin orchestrator for tests/dev, Validance workflow via the L3 integration directory) — generic Snakemake/Airflow framing dropped; § 2.5 *human-in-the-loop* rewritten to bind HITL to Validance's `ApprovalGate`, with the trace IDs corrected (UR-07 → SR-UI-09 / SR-OUT-05; UR-08 → SR-AS-09 / SR-UI-10; SR-IM-04 dropped — that SR is "improved rubric shall not contradict teaching material", unrelated to HITL) and the re-measurement loop made explicitly teacher-gated, never autonomous, with `Settings.max_iterations` (default 3) framed as a safety bound rather than a counter the system runs to. **§ 3 changes.** Decision-register rows #10 and #11 rewritten to summarise the four-layer split and Validance-as-recommended-runner; the locked-commitments paragraph after the table updated to point at the revised CLAUDE.md commitment #4. § 3.10 fully rewritten: the deliverable now ships L1 / L2 / L3 / L4 instead of "Python package + Vite static build + FastAPI server"; the Makefile target list reduced to `install / images / register / schemas / build / dev / test` (no `make run` — there is no custom HTTP server in the deliverable); the rationale block explains why no FastAPI, why one CLI subcommand per stage, why Docker for L2, why `validance/` is a separate top-level directory, why a custom Vite + React + shadcn + Tailwind SPA for L4, and why `make run` is absent. § 3.11 fully rewritten: from "no orchestration layer embedded" to "Validance is the recommended runner, the L3 directory is the sole `validance-sdk` import site, the hosted dev-VM instance is the canonical demo target, another orchestrator could wrap L1 by writing its own integration directory analogous to L3"; the two demo paths and the contract conditions for Path B are spelled out. **§ 4 changes.** § 4.8 *AuditBundle* reframed as a typed *view* the L3 harvester populates from Validance's audit chain after a workflow run completes — **not** a shape L1 writes to disk; on Path A no `AuditBundle` is produced (CLI inspection is single-stage); the model definitions, the discriminated union, and the index/detail split are unchanged but their on-disk realization is now owned by L3 (DR-INT-05) rather than by L1's `audit` sub-package; the privacy note rewritten so the bundle now lives inside Validance's run storage on the hosted dev-VM instance, never under a local `runs/<run_id>/` tree. The § 4.9 *ExplainedRubricFile* rationale paragraph updated to record that the deliverable is the same shape on both paths and is bundle-independent (Path A's `render` CLI subcommand produces it from in-process stage outputs; Path B's L3 harvester reads it from the workflow's terminal-task output). **§ 5 changes.** § 5.1 *DR-ARC* surgically updated: DR-ARC-01 drops the `api` sub-package and notes that L3 `validance/` and L4 `frontend/` live at the repo top level; DR-ARC-02 drops the `api` dependency edge; DR-ARC-04 reframes the in-process orchestrator as a thin chain used for tests, local development, and the `run-pipeline` CLI subcommand only — explicitly *not* a production execution surface; DR-ARC-06 reframes the `audit` sub-package as the producer of structured operation events rather than the writer of any cross-run audit chain (case (a) emit to stderr on Path A; case (b) feed the L3 harvester on Path B); DR-ARC-07 **repurposed** from "thin FastAPI server" to "**L1 has zero Validance imports**" (verifiable by a single grep over the L1 source tree and by the L1 `pyproject.toml` dependency list — the in-code realization of locked commitment #4); DR-ARC-08 strengthened — `grading-rubric-cli` now has one console-script subcommand per pipeline stage (`ingest`, `parse-inputs`, `assess`, `propose`, `score`, `render`, plus `run-pipeline` for the in-process chain); DR-ARC-10 reframed — the L4 SPA talks to Validance's REST API, not to a custom L1 api package; DR-ARC-11 wording updated ("no FastAPI process required" → "no in-process orchestrator and no Validance instance required"). § 5.3 *DR-DAT* surgically updated: DR-DAT-07 rewritten — L1 stages write **only structured stage outputs to user-specified `--output` paths**; the `runs/<run_id>/` layout is gone; the `render` subcommand is the sole writer of the `ExplainedRubricFile`; the audit-view bundle's on-disk shape is owned by L3 (§ 5.12 *DR-INT*). DR-DAT-07a rewritten to apply the index/detail invariant to the harvested view on Path B and to note explicitly that on Path A no bundle is produced (the invariant has no subject). DR-DAT-08 rewritten to apply atomic writes to any CLI `--output` path (and the harvester). DR-DAT-09 rewritten to drop the `runs/<run_id>/` framing — L1 has no in-process state across calls; multi-run coordination is the orchestrator's job. DR-DAT-10 rewritten to drop `Settings.audit_bundle_dir`; `Settings` no longer declares any output directory layout. **§ 5.11 *DR-DEP* filled (9 DRs).** Locks the four-layer artefact set: DR-DEP-01 the four top-level roots (`grading_rubric/`, `docker/`, `validance/`, `frontend/`); DR-DEP-02 L1 packaged as a standard Python distribution with `grading-rubric-cli` console-script and **no `validance-sdk` dependency**; DR-DEP-03 L2 Docker images self-contained with `ENTRYPOINT ["grading-rubric-cli"]`; DR-DEP-04 L4 SPA built separately, talks to Validance REST, never imports L1; DR-DEP-05 the Makefile surface (`install / images / register / schemas / build / dev / test`, no `make run`); DR-DEP-06 the two demo paths (`docker run` for Path A, `make register` + the SPA for Path B); DR-DEP-07 the three-command-to-Path-A target; DR-DEP-08 `.env.example` as the env-variable single source of truth; DR-DEP-09 the CI surface (unit tests + schema drift check + zero-Validance-imports static check). **§ 5.12 *DR-INT* added — NEW group, 9 DRs (8 Must / 1 Should).** This is the only place in § 5 where Validance vocabulary appears in normative DR-table form. DR-INT-01 locks the L3 directory layout (`validance/workflow.py` + `register.py` + `proposals.py` + `harvester.py`) as the **sole** site that imports `validance-sdk`. DR-INT-02 locks the workflow shape: `grading_rubric.assess_and_improve` with one Validance task per L1 stage (`ingest` → `parse_inputs` → `assess` → `propose` → `approve` (ApprovalGate) → `score` → `render`); each task is `docker run <image> grading-rubric-cli <stage> --input … --output …`; no L1 Python is called in-process from L3. DR-INT-03 locks the L1↔L3 audit contract: structured-JSON stderr operation events plus Validance's own audit chain — no private file format crosses the boundary. DR-INT-04 locks the proposal-payload mapping for `ProposedChange` (pure function, unit-testable without a Validance instance, both directions for the teacher decision round-trip). DR-INT-05 locks the harvester contract: `harvest_audit_bundle(run_id, validance_client) -> AuditBundle` is the sole producer of the audit-view bundle on Path B; invoked by Validance's REST API on demand; the SPA never sees raw audit-chain rows. DR-INT-06 locks HITL to Validance's `ApprovalGate` between `propose` and `score`, with the iteration loop teacher-gated and `Settings.max_iterations` framed as a safety bound, never autonomous (matches the corrected § 2.5 framing). DR-INT-07 locks `register.py` idempotency. DR-INT-08 (Should) names the contract a different orchestrator must reproduce to wrap L1 (one task per stage + structured stderr + `ProposedChange` mapping + a harvester producing the same `AuditBundle` shape) — L1 itself stays unchanged. DR-INT-09 (Should) keeps the L4 codegen surface restricted to L1 `models`. **§ 1 / § 5 intro / status / scope updated** to count twelve DR groups, name the four-layer architecture, and reflect that DR-DEP and DR-INT are now filled. § 6 traceability sweep deferred to the round that closes out the remaining DR-LLM / DR-AS / DR-IM / DR-UI / DR-IO / DR-OBS / DR-PER / DR-MOD groups. The pivot note (`notes/validance-pivot.md`) remains the source of truth for the rationale chain that drove this fold-in; it will be archived to `notes/dr-r-pivot-final.md` (or deleted) once the next DR cluster lands. |
