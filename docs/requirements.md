...
System requirements are split into two sub-sections at the same abstraction layer:
...
Non-functional system requirements specify *how well* the system must do what it does: quality attributes such as observability and performance. They live at the same abstraction layer as functional system requirements and trace to the same user requirements, but they constrain the system's qualities rather than its behaviors.
...
| 0.4.0 | 2026-04-10 | Wiktor Lisowski | Restructured § 5 *System Requirements* into two sub-sections at the same abstraction layer: § 5.1 *Functional system requirements* (SR-IN, SR-AS, SR-IM, SR-UI, SR-OUT — 38 SRs) and § 5.2 *Non-functional system requirements* (SR-OBS, SR-PRF — 6 SRs). Group sub-sub-sections renumbered (5.1 → 5.1.1, …, 5.7 → 5.2.2). Intent: make the functional / non-functional split explicit without inventing a top-level § 6 that would imply NFRs sit at a different abstraction layer than functional SRs (they don't). No requirements added, removed, modified or re-traced. |
